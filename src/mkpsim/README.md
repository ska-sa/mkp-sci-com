...
Run `pip install .` from the  the project [root folder](../../). This will install the `mkpsim` subpackage and neccesary modules.

## Running the simulation

### Required Inputs

* [MeerKAT+ Holography Beam models](https://drive.google.com/drive/folders/1otR21f4uGo2WUBH5HmudoNO142TRpnpX?usp=sharing). Dowload and save them somewhere.
* Point Source Catalog. [Tigger format](https://github.com/ratt-ru/tigger-lsm/blob/ab868c7db8423d7153d0c2eb48c7ca29fd84d287/Tigger/Models/Formats/ASCII.py#L45-L87) source catalog ASCII file.

## Running the simulation

Run the following command
```bash
stimela run mkpsim::mkpsim outdir=<output-directory> skymodel=<ASCII-source-catalog> antpos=src/mkpsim/configs/meerkat_plus.itrf.txt mkp-beam-file=<meerkat-plus-beam-fits> sim-config=src/mkpsim/configs/meqtree_config.tdl imsize=<quick-image-size> imscale=<quick-image-resolution>
```
...
- **Beam models:** Beam fits and analytic models can be supplied; holography-derived beams are included for experimentation. The holography beam file can be large and is commonly provided by the user; you can point the recipe at a local file with the `beam-file` input (default: `{recipe.dir.base}/data/MK+L_sym.fits`).
