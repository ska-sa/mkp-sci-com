...
eidos -p 256 -d 4 -f 1300 1350 5 \
    -P beam_models/mk.eidos.lband.${start_freq}MHz.4deg -o8

# Split holographic beam into meqtree format
python scripts/split_beam.py --freq_range=[1290,1310] --prefix=mkp.holo.lband.1290MHz beam_models/MK+L_sym.fits

Run stimela
stimela --backend singularity run cultcargo::meqtree-pipeliner \
    ms=${ms} skymodel=single_point.txt \
